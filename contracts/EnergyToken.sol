...
/// @dev for now i use a enregy to tokenID mapping but it could make sense to have the energy amount == tokenID such that the energy that it represents can be viewed in the envent emitted.
    function addEnergyBatch(uint _energy, uint _amount, uint _price) external onlyOwner{
        ...
        tokenPrice[energyToToken[_energy]] = _price; 
     ...
     */
    function safeBuy(uint256 id,uint256 amount) public payable {
        require( msg.value == tokenPrice[id]*amount, "wrong value sent");
        address from = owner();
    ...
    function getPrice(uint _id, uint _amount) public view returns(uint){ 
        return tokenPrice[_id]*_amount;
    }
