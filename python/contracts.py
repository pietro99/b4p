...
#@NOTE gas values and gas prices someimes cause issues. needs implementatoin of gas strategy.
class ContractManager(Connect):
    def __init__(self, conn):
        super().__init__(conn)
        self.contracts_names = ["EnergyToken", "ERC20"]#@BUG: doesn't work if contract name != contract file name
    ...
    def deploy(self, contract_name,wallet, verbose=False,**kwargs):
        args = kwargs.values()
        try:
        ...
        transaction_dict["gas"] = gas #@NOTE: using gas value will run out of gas so i use gas*2
        print(gas)
        print(f"gas price: {self.web3.eth.gas_price}")
        transaction_dict["gasPrice"] = 30000000000
        print(f"gas price: {30000000000}\nestimated gas: {gas}")
        ...
        if verbose:
            print(f"\ndeploying {contract_name}\ntransaction hash -->"+str(tx_receipt["transactionHash"].hex()))
        return tx_receipt
    ...
    def call(self,contract,function_name, wallet,verbose=False, **kwargs):
        value = kwargs.pop('value') if "value" in kwargs.keys() else 0
        denomination = kwargs.pop('denomination') if "denomination" in kwargs.keys() else "wei"

        args = kwargs.values()
        transaction_dict = {}
        transaction_dict["from"] = wallet.account.address
        transaction_dict["value"] = self.web3.toWei(value, denomination)
    ...
    def read(self,contract,function_name,verbose=False,**kwargs):
        args = kwargs.values()
        transaction_dict = {}
        transaction_dict["chainId"] = self.chain_id
        transaction_dict["gasPrice"] = self.web3.eth.gas_price
